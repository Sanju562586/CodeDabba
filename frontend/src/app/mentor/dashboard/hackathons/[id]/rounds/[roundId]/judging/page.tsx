...
export default function MentorJudgingPage() {
    ...
    const handleSubmitScore = async (team: Team) => {
        ...
        if (!submission) {
            toast.error("No transmission found for this squad");
            return;
        }
    ...
    const latestSub = selectedTeam?.submissions?.[0];
    const isEvalPhase = hackathon?.status === 'ROUND_EVALUATION';
    const now = new Date();
    const hasStarted = !round?.evaluationStart || now >= new Date(round.evaluationStart);
    const hasNotEnded = !round?.evaluationEnd || now <= new Date(round.evaluationEnd);
    const isWithinWindow = hasStarted && hasNotEnded;
    const canEvaluate = isEvalPhase && isWithinWindow && !!latestSub;
                    ...
                    <div className="flex items-center gap-6 mb-12">
                        ...
                        <div>
                            <div className="flex items-center gap-3">
                                ...
                                <span className="px-3 py-1 bg-zinc-800 text-[10px] font-black uppercase text-emerald-500 rounded-lg">Full historical visibility enabled</span>
                            </div>
                                        ...
                                        <div className="flex justify-between items-start mb-10">
                                            ...
                                            {latestSub && (
                                                <div className="p-3 bg-zinc-800 rounded-2xl border border-zinc-700">
                                                    <span className="text-[10px] font-black text-zinc-400">SUBMISSION RECORD</span>
                                                </div>
                                            )}
                                        </div>
                                    ...
                                    <section className="grid md:grid-cols-3 gap-8">
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <button
                                                ...
                                                disabled={processing || !canEvaluate}
                                            ...
                                            >
                                                ...
                                                {!latestSub ? "No submission" : canEvaluate ? "Scoring enabled (Transmit)" : "Scoring locked"}
                                            </button>
                                        </div>
                                    </section>
                                </div>
