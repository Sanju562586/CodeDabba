        ...
        <div className="min-h-screen bg-black text-white selection:bg-pink-500 selection:text-white pb-24">
                ...
                <div className="pt-24 pb-12 px-6 container mx-auto">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center gap-4">
                            ...
                            <span className={`px-4 py-1 text-[10px] font-black rounded-full uppercase tracking-widest border transition-all shadow-lg ${
                                ...
                                {hackathon?.status === 'registration_open' ? 'REGISTRATION' : 
                                 ...
                                 hackathon?.status === 'completed' ? 'HACKATHON COMPLETED' : 
                    ...
                    <div className="min-h-[400px]">
                        ...
                        {activeTab === 'teams' && <TeamsTab hackathonId={id as string} hackathonStatus={hackathon.status} />}
...
function TeamsTab({ hackathonId, hackathonStatus }: { hackathonId: string, hackathonStatus?: string }) {
    ...
    const filteredTeams = teams.filter(t => {
        ...
        else if (filter === 'approved') statusMatch = ['approved', 'winner'].includes(t.status);
                                ...
                                >
                                    ...
                                    <td className="px-8 py-8">
                                        <span className={`px-4 py-1.5 text-[9px] font-black rounded-lg uppercase tracking-[0.2em] border transition-all flex items-center gap-2 w-fit ${
                                            ...
                                            team.status === 'winner' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20 shadow-lg shadow-amber-500/5' :
                                            ...
                                            {team.status === 'pending_approval' ? 'Pending' :
                                             team.status === 'winner' ? (hackathonStatus?.toUpperCase() === 'COMPLETED' ? 'Winner (Hackathon Completed)' : 'Winner') :
                                              team.status === 'approved' ? (hackathonStatus?.toUpperCase() === 'COMPLETED' ? 'Approved (Hackathon Completed)' : 'Approved') : 
                                              (hackathonStatus?.toUpperCase() === 'COMPLETED' ? 'Eliminated (Hackathon Completed)' : 'Eliminated')}
                                        </span>
                                    </td>
                            ...
                            <tr key={entry.teamId} className="group hover:bg-zinc-800/30 transition-all duration-300 cursor-default">
                                ...
                                <td className="px-8 py-6">
                                    <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${
                                        entry.status === 'winner' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20 shadow-lg shadow-amber-500/5' :
...
function ActivityTab({ hackathonId }: { hackathonId: string }) {
    ...
    const fetchActivities = useCallback(async () => {
        try {
            const res = await api.get(`/hackathons/${hackathonId}/admin-activities`);
            setActivities(res.data);
        } catch (error) {
            console.error("Scroll Sync Failure", error);
        } finally {
            setLoading(false);
        }
    }, [hackathonId]);
    ...
    useEffect(() => {
        fetchActivities();
        // Tactical Real-time Polling (Every 15s)
    ...
    const filtered = activities.filter(a => {
        ...
        if (filter === 'SECURITY') return ['security_alert', 'unauthorized_access', 'api_failure'].includes(a.activityType);
        if (filter === 'PROTOCOL') return ['phase_transition', 'hackathon_updated'].includes(a.activityType);
        if (filter === 'SQUAD') return ['user_joined', 'team_created', 'team_approved', 'team_rejected'].includes(a.activityType);
        return true;
    });
    ...
    if (loading && activities.length === 0) return (
        <div className="p-24 text-center">
            ...
            <p className="text-[10px] font-black uppercase text-zinc-500 tracking-widest animate-pulse tracking-[0.3em]">Decoding Scroll of Truth...</p>
        </div>
    );

    return (
        <div className="bg-zinc-950/50 border border-zinc-800/50 rounded-[2.5rem] p-10 shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden backdrop-blur-md">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-3 mt-3">
                         <div className="flex items-center gap-1.5 px-2.5 py-1 bg-green-500/10 border border-green-500/20 rounded-full">
                            ...
                            <span className="text-[9px] font-black text-green-500 uppercase tracking-widest leading-none">Live Protocol Monitoring</span>
                         </div>
                         <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Immutable Combat Log</p>
                    </div>
                </div>
                
                <div className="flex bg-zinc-900/50 border border-zinc-800 p-1.5 rounded-2xl gap-1">
                    {['ALL', 'PROTOCOL', 'SQUAD', 'SECURITY'].map((f) => (
                        <button 
                            key={f}
                            onClick={() => setFilter(f)}
                            className={`px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-[0.1em] transition-all ${
                                filter === f ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-zinc-500 hover:text-zinc-300'
                            }`}
                        >
                            {f}
                        </button>
            ...
            <div className="relative">
                {/* Timeline Line */}
                <div className="absolute left-[3.25rem] top-0 bottom-0 w-[1px] bg-gradient-to-b from-zinc-800 via-indigo-500/20 to-zinc-800"></div>
                ...
                <div className="space-y-4 relative">
                    {filtered.map((a, idx) => (
                        <div key={a.id} className="flex gap-8 relative group">
                            ...
                            <div className="w-20 pt-1 flex flex-col items-end">
                                <span className="text-[10px] font-black text-zinc-600 uppercase tracking-tighter group-hover:text-zinc-400 transition-colors">
                                    {formatDate(new Date(a.createdAt), 'MMM dd')}
                                </span>
                                <span className="text-[12px] font-mono font-medium text-zinc-500 tabular-nums">
                                    {formatDate(new Date(a.createdAt), 'HH:mm:ss')}
                                </span>
                            </div>

                            {/* Node */}
                            <div className="relative z-10 flex items-start pt-1">
                                <div className={`w-10 h-10 rounded-xl border-2 flex items-center justify-center transition-all group-hover:scale-110 ${
                                    a.status === 'blocked' ? 'bg-red-500/10 border-red-500 text-red-500 shadow-[0_0_15px_rgba(239,68,68,0.2)]' :
                                    a.status === 'alert' ? 'bg-amber-500/10 border-amber-500 text-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.2)]' :
                                    a.activityType.includes('transition') ? 'bg-indigo-500/10 border-indigo-500 text-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.2)]' :
                                    'bg-zinc-900 border-zinc-700 text-zinc-400'
                                }`}>
                                    {a.status === 'blocked' ? <ShieldAlert className="w-4 h-4" /> :
                                     a.activityType.includes('submission') ? <Send className="w-4 h-4" /> :
                                     a.activityType.includes('score') ? <Award className="w-4 h-4" /> :
                                     a.activityType.includes('phase') ? <Zap className="w-4 h-4" /> :
                                     <Activity className="w-4 h-4" />}
                                </div>
                            </div>

                            {/* Content */}
                            <div className="flex-1 bg-zinc-900/30 border border-white/[0.03] hover:bg-white/[0.03] transition-all rounded-[1.5rem] p-6 mb-4 group-hover:border-white/[0.08]">
                                <div className="flex flex-wrap items-center justify-between gap-4 mb-3">
                                    <div className="flex flex-wrap items-center gap-3">
                                        <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${
                                            a.phase?.includes('REGISTRATION') ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' :
                                            a.phase?.includes('ROUND') ? 'bg-pink-500/10 text-pink-500 border-pink-500/20' :
                                            'bg-zinc-800 text-zinc-500 border-zinc-700'
                                        }`}>
                                            {a.phase || 'General'}
                                        </span>
                                        <h4 className="text-sm font-bold text-zinc-100 uppercase tracking-tight group-hover:text-white transition-colors">{a.description}</h4>
                                    </div>
                                    
                                    <div className="flex items-center gap-3">
                                        {a.status && (
                                            <span className={`px-2.5 py-1 rounded-md text-[8px] font-black uppercase tracking-[0.2em] shadow-sm ${
                                                a.status === 'success' ? 'bg-green-500/10 text-green-500 border border-green-500/20' :
                                                a.status === 'blocked' ? 'bg-red-500/10 text-red-500 border border-red-500/20' :
                                ...
                                <div className="flex items-center gap-6">
                                    <div className="flex items-center gap-2">
                                        <div className="w-6 h-6 rounded-lg bg-zinc-950 border border-zinc-800 flex items-center justify-center">
                                            <User className="w-3 h-3 text-zinc-500" />
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-zinc-300 uppercase leading-none">{a.actorName || a.performedBy?.name || 'Protocol Core (SYSTEM)'}</p>
                                            <p className="text-[8px] font-bold text-zinc-600 uppercase tracking-widest mt-1">{a.actorRole || a.performedBy?.role || 'System'}</p>
                                        </div>
                                    </div>
                                    ...
                                    {a.ipAddress && (
                                        <div className="flex items-center gap-2 text-zinc-600">
                                            ...
                                            <span className="text-[9px] font-mono tracking-tighter">{a.ipAddress}</span>
                                        </div>
                                    )}
                                </div>

                                {/* Metadata Details (Before/After) */}
                                {a.metadata && (
                                    <div className="mt-5 p-4 bg-black/40 border border-zinc-800/50 rounded-xl overflow-x-auto">
                                        <div className="flex flex-col gap-3">
                                            {a.metadata.prevState && a.metadata.newState ? (
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div>
                                                        <p className="text-[8px] font-black text-zinc-600 uppercase mb-2 tracking-widest">Protocol Pre-State</p>
                                                        <pre className="text-[10px] font-mono text-zinc-500 bg-red-500/5 p-2 rounded-lg border border-red-500/10">
                                                            {typeof a.metadata.prevState === 'object' ? JSON.stringify(a.metadata.prevState, null, 2) : String(a.metadata.prevState)}
                                                        </pre>
                                                    </div>
                                                    <div>
                                                        <p className="text-[8px] font-black text-zinc-600 uppercase mb-2 tracking-widest">Protocol Post-State</p>
                                                        <pre className="text-[10px] font-mono text-indigo-400 bg-indigo-500/5 p-2 rounded-lg border border-indigo-500/10">
                                                            {typeof a.metadata.newState === 'object' ? JSON.stringify(a.metadata.newState, null, 2) : String(a.metadata.newState)}
                                                        </pre>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div>
                                                    <p className="text-[8px] font-black text-zinc-600 uppercase mb-2 tracking-widest">Protocol Telemetry</p>
                    ...
                    {filtered.length === 0 && (
                        <div className="py-32 text-center relative z-10">
                            <div className="w-16 h-16 bg-zinc-900 border border-zinc-800 rounded-2xl flex items-center justify-center mx-auto mb-8 animate-pulse text-zinc-700">
                                <Shield className="w-8 h-8" />
                            </div>
                            <h4 className="text-xl font-bold text-zinc-600 uppercase tracking-tighter">The archives are sealed</h4>
                            <p className="text-[10px] font-black text-zinc-700 mt-3 uppercase tracking-[0.3em] italic">No Protocol logs found for current encryption key.</p>
                        </div>
                    )}
                </div>
            </div>
