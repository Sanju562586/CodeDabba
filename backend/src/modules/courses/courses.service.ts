...
export class CoursesService {
  ...
  async createCourse(
    ...
    if (createCourseDto.accessType === CourseAccessType.FREE) {
      ...
      createCourseDto.accessType === CourseAccessType.PAID &&
      createCourseDto.price <= 0
    ) {
    ...
    // Slug Generation
    const slug = await this.generateSlug(createCourseDto.title);
  ...
  async findAll(
    ...
    const dataWithEnrollment = await Promise.all(
  ...
  async enroll(userId: string, courseId: string, skipPaymentCheck = false): Promise<Enrollment> {
    ...
    if (course.modules && course.modules.length > 0) {
      ...
      if (firstModule.chapters && firstModule.chapters.length > 0) {
        const sortedChapters = firstModule.chapters.sort(
          (a, b) => a.orderIndex - b.orderIndex,
        );
    ...
    if (userId) {
      ...
      if (isEnrolled) {
        ...
        if (prog) {
          ...
          const percentage =
            totalChapters > 0
              ? Math.round((prog.completedLessonsCount / totalChapters) * 100)
              : 0;
    ...
    const coursesWithProgress = await Promise.all(
      enrollments.map(async (enrollment) => {
        ...
        const percentage =
          totalChapters > 0
            ? Math.round(
                ((progress?.completedLessonsCount || 0) / totalChapters) * 100,
              )
            : 0;
  ...
  async createModule(
    ...
    if (
      ...
      throw new ForbiddenException(
        `Cannot add modules while course is ${course.status.replace(/_/g, ' ')}.`,
      );
    }

    // If structure changes after curriculum is approved/content is drafted -> Reset to curriculum draft
    if (
      [CourseStatus.CURRICULUM_APPROVED, CourseStatus.CONTENT_DRAFT].includes(
        course.status,
      )
    ) {
    ...
    if (existingModule) {
      throw new ConflictException(
        `Module with order index ${createModuleDto.orderIndex} already exists`,
      );
    }
  ...
  async createChapter(
    ...
    if (
      ...
      throw new ForbiddenException(
        `Cannot add chapters while course is ${course.status.replace(/_/g, ' ')}.`,
      );
    }
    ...
    if (existingChapter) {
      throw new ConflictException(
        `Chapter with order index ${createChapterDto.orderIndex} already exists`,
      );
    }
  ...
  async createBlock(
    ...
    if (lockedStatuses.includes(course.status)) {
      throw new ForbiddenException(
        `Cannot edit content while course is ${course.status.replace(/_/g, ' ')}.`,
      );
    }
    ...
    if (existingBlock) {
      throw new ConflictException(
        `Block with order index ${createBlockDto.orderIndex} already exists`,
      );
    }
  ...
  async reorderModules(
    ...
    if (
      ...
      throw new ForbiddenException(
        `Cannot reorder modules while course is ${course.status.replace(/_/g, ' ')}.`,
      );
    }
    ...
    await this.modulesRepository.manager.transaction(
      async (transactionalEntityManager) => {
        for (const item of items) {
          ...
          if (result.affected === 0) {
            throw new BadRequestException(
              `Module ${item.id} does not belong to course ${courseId} or does not exist`,
            );
          }
        }
      },
  ...
  async reorderChapters(
    ...
    if (
      ...
      throw new ForbiddenException(
        `Cannot reorder chapters while course is ${course.status.replace(/_/g, ' ')}.`,
      );
    }
    ...
    await this.chaptersRepository.manager.transaction(
      async (transactionalEntityManager) => {
        for (const item of items) {
          const result = await transactionalEntityManager.update(
            Chapter,
          ...
          if (result.affected === 0) {
            throw new BadRequestException(
              `Chapter ${item.id} does not belong to module ${moduleId} or does not exist`,
            );
          }
        }
      },
  ...
  async reorderBlocks(
    ...
    if (!allowedContentEditStatuses.includes(course.status)) {
      throw new ForbiddenException(
        'Cannot edit content (reorder blocks). Check course status.',
      );
    }
    ...
    await this.blocksRepository.manager.transaction(
      async (transactionalEntityManager) => {
        for (const item of items) {
          const result = await transactionalEntityManager.update(
            LessonBlock,
          ...
          if (result.affected === 0) {
            throw new BadRequestException(
              `Block ${item.id} does not belong to chapter ${chapterId} or does not exist`,
            );
          }
        }
      },
  ...
  async submitCurriculum(user: User, courseId: string): Promise<Course> {
    ...
    if (course.status !== CourseStatus.DRAFT_CURRICULUM) {
      throw new BadRequestException(
        'Can only submit curriculum from DRAFT CURRICULUM status',
      );
    }
  ...
  async toggleChapterFreeStatus(
    ...
    if (
      [CourseStatus.DRAFT_CURRICULUM, CourseStatus.CURRICULUM_REJECTED].indexOf(
        course.status,
      ) === -1
    ) {
      ...
      throw new ForbiddenException(
        'Cannot change free preview flags after curriculum approval',
      );
    }
  ...
  async completeChapter(
    ...
    if (chapter.module.courseId !== courseId)
      throw new BadRequestException('Chapter does not belong to this course');
        ...
        percentage: Math.min(
          ...
          Math.round(
            (progress.completedLessonsCount /
              ((await this.getCourseTotalChapters(courseId)) || 1)) *
              100,
          ),
        ),
        currentChapterId: progress.currentChapterId,
  ...
  private async generateSlug(title: string): Promise<string> {
    ...
    while (
      await this.coursesRepository.findOne({ where: { slug: uniqueSlug } })
    ) {
