...
export class CoursesController {
  ...
  async uploadThumbnail(
    ...
    return await this.coursesService.getUploadUrl(
      body.filename,
      body.contentType,
    );
  }
  ...
  async toggleChapterFreeStatus(
    @Request() req: any,
    @Param('chapterId') chapterId: string,
    @Body() body: { isFreePreview: boolean },
  ) {
    // Enforce boolean if not done automatically
    return await this.coursesService.toggleChapterFreeStatus(
      req.user,
      chapterId,
      !!body.isFreePreview,
    );
  ...
  async completeChapter(
    ...
    return await this.coursesService.completeChapter(
      req.user,
      courseId,
      chapterId,
    );
