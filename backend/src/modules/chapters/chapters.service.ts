...
export class ChaptersService {
  ...
  async createBlock(
    chapterId: string,
    createBlockDto: CreateLessonBlockDto,
    userId: string,
  ): Promise<LessonBlock> {
    ...
    if (!allowedStatuses.includes(course.status)) {
      throw new BadRequestException(
        'Cannot add blocks. Curriculum must be approved first or content in draft/rejected.',
      );
    }
  ...
  async findOne(id: string, user: any): Promise<Chapter> {
    ...
    if (course.mentorId !== user?.id) {
      throw new ForbiddenException(
        'You are not authorized to view/edit this chapter',
      );
    }
  ...
  async getChapterContentForStudent(
    chapterId: string,
    userId?: string,
  ): Promise<Chapter> {
    const chapter = await this.chaptersRepository.findOne({
    ...
    if (!userId) {
      throw new ForbiddenException(
        'Identification required to access this content',
      );
    }
